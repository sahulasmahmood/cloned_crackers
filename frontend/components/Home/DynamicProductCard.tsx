...
export default function DynamicProductCard({
  ...
  const handleDecrement = (e: React.MouseEvent) => {
    ...
    updateQuantity(product.id, inventoryProductId, quantity - 1, selectedCuttingStyle || undefined);
    if (quantity === 1) {
      toast.info("Item removed from enquiry list");
    }
  };
      ...
      <div className="p-3 sm:p-3 flex flex-col flex-1">
        ...
        <div ref={dropdownRef} className="relative mb-2 sm:mb-2 h-9 sm:h-8">
          <button
            ...
            className={`w-full h-full flex items-center justify-between px-3 sm:px-3 rounded text-sm sm:text-sm text-gray-700 transition-colors ${activeVariants.length === 1
          ...
          >
            ...
            {activeVariants.length > 1 && (
              <IconChevronDown
                size={14}
                className={`flex-shrink-0 transition-transform sm:w-3.5 sm:h-3.5 ${showVariants ? "rotate-180" : ""
                  }`}
              />
            )}
          </button>
              ...
              {activeVariants.map((variant, index) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full px-3 sm:px-3 py-2 sm:py-1.5 text-left hover:bg-gray-50 flex items-center justify-between gap-2 ${selectedVariant === index ? "bg-green-50" : ""
                      }`}
                  >
        ...
        {hasCuttingStyles && (
          <div ref={cuttingStyleRef} className="relative mb-2 sm:mb-2">
            ...
            >
              ...
              <IconChevronDown
                size={12}
                className={`sm:w-3 sm:h-3 transition-transform ${showCuttingStyles ? "rotate-180" : ""
                  }`}
              />
            </button>
              ...
              <div className="absolute top-full left-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50 min-w-[160px] max-h-40 overflow-y-auto">
                ...
                {cuttingStyles.map((style) => (
                  <button
                    ...
                    className={`w-full px-3 sm:px-3 py-2 sm:py-2 text-left hover:bg-gray-50 flex items-center justify-between border-b border-gray-100 last:border-0 ${selectedCuttingStyle === style.name ? "bg-green-50" : ""
                      }`}
                  >
