 ...
 * Data Sources: Lead
 */
async function getLeadsData(startDate, endDate) {
  try {
    ...
    // Calculate leads in current period
    const periodLeads = await prisma.lead.count({
    ...
    return {
      ...
      recent: recentLeads,
      periodTotal: periodLeads
    };
  } catch (error) {
    console.error("❌ Error in getLeadsData:", error);
    return { total: 0, new: 0, converted: 0, recent: [], periodTotal: 0 };
  }
}
...
async function getLicenseData() {
  try {
    ...
    const expired = licenses.filter(l => l.expiryDate < today).length;

    const active = licenses.filter(l => l.isActive).length;
    ...
    return {
      ...
      details: licenses.map(l => ({
        ...
        status: l.expiryDate < today ? 'expired' : (l.expiryDate <= thirtyDaysFromNow ? 'expiring' : 'valid'),
        daysToExpiry: Math.ceil((new Date(l.expiryDate) - today) / (1000 * 60 * 60 * 24))
      }))
    };
  } catch (error) {
    console.error("❌ Error in getLicenseData:", error);
    return { total: 0, active: 0, expiringSoon: 0, expired: 0, details: [] };
  }
}
